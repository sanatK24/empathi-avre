...
const CommentsModal = ({ update, campaignId, onClose, onCommentAdded }) => {
  ...
  const fetchComments = async () => {
    try {
      setLoading(true);
      const data = await apiService.getUpdateComments(
        profile.accessToken,
        campaignId,
        update.id
      );
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      const comment = await apiService.addUpdateComment(
        profile.accessToken,
        campaignId,
        update.id,
        { text: newComment }
  ...
  const handleDeleteComment = async (commentId) => {
    if (window.confirm('Delete this comment?')) {
      try {
        await apiService.deleteUpdateComment(
          profile.accessToken,
          campaignId,
          update.id,
          commentId
              ...
              >
                ...
                {profile.userId === comment.user_id && (
                  <button
                    ...
                    className="p-2 hover:bg-red-50 rounded-lg transition-colors text-red-600 opacity-0 group-hover:opacity-100"
                  >
                    <Trash2 size={16} />
                  </button>
                )}
          ...
          >
            ...
            <div className="flex-1 flex gap-2">
              <input
                ...
                maxLength={500}
                className="flex-1 px-4 py-2 border border-slate-200 rounded-full focus:outline-none focus:ring-2 focus:ring-primary-500"
              />
              <button
                type="submit"
