  ...
  useEffect(() => {
    const fetchCampaign = async () => {
      try {
        ...
        const saved = localStorage.getItem(`campaignEditData_${id}`);
        let localData = null;
        if (saved) { try { localData = JSON.parse(saved); } catch(e) {} }
  ...
  useEffect(() => {
    const fetchTaxonomy = async () => {
      try {
        ...
        setTaxonomy(active);
        if (active.length > 0) {
          setFormData(prev => ({ ...prev, category_id: active[0].id, subcategory_id: active[0].subcategories[0]?.id || '' }));
        }
      ...
      try {
        ...
        setFormData(prev => {
          ...
          if (res.predicted_subcategory) {
            ...
            if (mc) {
              const ms = mc.subcategories.find(s => s.name.toLowerCase() === res.predicted_subcategory.toLowerCase());
              if (ms) u.subcategory_id = ms.id.toString();
            }
          }
          return u;
        });
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
      <div className="lg:col-span-2">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-5 md:p-8 space-y-4 md:space-y-6">
          ...
          <div>
            <label className={LABEL_CLS}>Verification Documents (Optional)</label>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 overflow-hidden">
                  <p className="text-sm font-medium text-slate-900 truncate">
                    {verificationDocument ? verificationDocument.name : formData.verification_doc_url ? 'Existing document uploaded' : 'Select a document to upload...'}
                  </p>
                  <p className="text-xs text-slate-500">
                    {verificationDocument ? `${(verificationDocument.size / 1024 / 1024).toFixed(2)} MB`
                      : formData.verification_doc_url ? <a href={formData.verification_doc_url} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:underline z-10 relative">View current document</a>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {aiFieldConfig.map(({ name, label, showAi }) => (
              <div key={name}>
                <label className={LABEL_CLS}>{label} <span className="text-red-600">*</span></label>
                <div className="relative">
                  <input type="text" name={name} value={formData[name]} onChange={handleInputChange} placeholder="Awaiting analysis..." className={`${INPUT_CLS} bg-white`} />
                  {showAi && (
            ...
            <div>
              <label className={LABEL_CLS}>Urgency Level <span className="text-red-600">*</span></label>
              <select name="urgency_level" value={formData.urgency_level} onChange={handleInputChange} className={INPUT_CLS}>
                {URGENCIES.map(u => <option key={u} value={u}>{u.charAt(0).toUpperCase() + u.slice(1)}</option>)}
              </select>
            </div>
            {gridFields.map(({ name, label, required, ...rest }) => (
              <div key={name}>
                <label className={LABEL_CLS}>{label} {required && <span className="text-red-600">*</span>}</label>
                <input name={name} value={formData[name]} onChange={handleInputChange} className={INPUT_CLS} {...rest} />
              </div>
            ))}
            <div className="md:col-span-2">
              <label className={LABEL_CLS}>Campaign Deadline (Optional)</label>
          ...
          <div className="flex gap-3 pt-6 border-t border-slate-200">
            ...
            <Button type="submit" disabled={loading} className="flex-1 bg-primary-600 hover:bg-primary-700 text-white font-bold shadow-lg shadow-primary-500/20 disabled:opacity-50 flex items-center justify-center gap-2">
              {loading ? <><Loader2 size={18} className="animate-spin" /> Creating...</> : 'Update Campaign'}
            </Button>
          </div>
        </form>
      </div>
