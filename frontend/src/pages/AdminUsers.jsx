...
const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    ...
    setError(null);
    try {
      const data = await apiService.getAdminUsers(profile.accessToken, 0, 200);
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter((u) => {
      ...
      const matchesQuery = !q || name.includes(q) || email.includes(q) || String(u.id || '').includes(q);
  ...
  const roleBadges = (user) => {
    ...
    if (!role) return <Badge variant="default">Unknown</Badge>;
    if (role === 'admin') return <Badge variant="success">Admin</Badge>;
    if (role === 'creator' || role === 'user' || role === 'requester') return <Badge variant="warning">{role}</Badge>;
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Button variant="secondary" size="md" onClick={fetchUsers} icon={null}>
          Refresh
        </Button>
      </div>
        ...
        <CardContent className="p-0">
          {filtered.length === 0 ? (
            <EmptyState
              ...
              description="Adjust filters or search for a different user."
              variant="dashed"
              className="py-16"
            />
          ) : (
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filtered.map((u) => {
                    ...
                    const status = u.is_active === false ? 'Inactive' : u.is_active ? 'Active' : 'Active';
                    return (
                      <tr key={u.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-5">{roleBadges(u)}</td>
                        <td className="px-6 py-5">
                          {status === 'Inactive' ? (
                            <Badge variant="danger">
                              <AlertTriangle className="w-3.5 h-3.5 inline mr-1" />Inactive
                            </Badge>
                          ) : (
