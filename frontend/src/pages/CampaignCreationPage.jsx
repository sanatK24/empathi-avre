...
const AiBadge = () => (
  <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
    <ShieldCheck className="w-4 h-4 text-indigo-500" /><span className="text-xs text-indigo-600 font-medium">AI</span>
  </div>
);
function CampaignCreationPage() {
  ...
  useEffect(() => {
    const fetchTaxonomy = async () => {
      try {
        ...
        setTaxonomy(active);
        if (active.length > 0) setFormData(prev => ({ ...prev, category_id: active[0].id, subcategory_id: active[0].subcategories[0]?.id || '' }));
  ...
  const triggerAnalysis = async () => {
    if (formData.title.trim().length > 5 || formData.description.trim().length > 20) {
      setAnalyzing(true);
      try {
        const res = await apiService.analyzeCampaign(profile.accessToken, {
          ...formData,
        ...
        setFormData(prev => {
          const u = { ...prev };
          if (res.extracted_goal) u.goal_amount = res.extracted_goal.toString();
          if (res.inferred_urgency) u.urgency_level = res.inferred_urgency;
          if (res.predicted_category) {
            u.category_name = res.predicted_category;
            const mc = taxonomy.find(c => c.name.toLowerCase() === res.predicted_category.toLowerCase());
            if (mc) u.category_id = mc.id.toString();
          }
          if (res.predicted_subcategory) {
            u.subcategory_name = res.predicted_subcategory;
            const mc = taxonomy.find(c => c.id === parseInt(u.category_id || formData.category_id));
            if (mc) { const ms = mc.subcategories.find(s => s.name.toLowerCase() === res.predicted_subcategory.toLowerCase()); if (ms) u.subcategory_id = ms.id.toString(); }
          }
          return u;
        });
      } catch(e) { console.warn("Auto-analyze failed:", e); }
  ...
  const handleRefineDescription = async () => {
    ...
    setRefining(true);
    try { const res = await apiService.refineCampaignDescription(profile.accessToken, formData.description); setFormData(prev => ({ ...prev, description: res.refined_description })); }
  ...
  const handleSubmit = async (e) => {
    ...
    for (const [cond, msg] of validations) { if (cond) { setError(msg); return; } }
    try {
    ...
    } catch (err) { console.error('Campaign creation failed:', err); setError(err.message || 'Failed to create campaign. Please try again.'); }
    finally { setLoading(false); }
  };
  const selectedCategory = taxonomy.find(c => c.id === parseInt(formData.category_id));
  const aiRules = selectedCategory?.ai_rules || [];
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
      <div className="lg:col-span-2">
        <div className="mb-6 md:mb-8">
          <button onClick={() => navigate('/user/campaigns')} className="flex items-center gap-2 text-primary-600 hover:text-primary-700 mb-3 md:mb-4 font-bold text-sm md:text-base">
            <ArrowLeft size={18} className="md:w-5 md:h-5" /> Back to Campaigns
          </button>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-5 md:p-8 space-y-4 md:space-y-6">
          <div>
            <label className="block text-xs md:text-sm font-semibold text-slate-900 mb-1.5 md:mb-2">Campaign Title <span className="text-red-600">*</span></label>
            <input type="text" name="title" value={formData.title} onChange={handleInputChange} placeholder="Enter campaign title" maxLength="200" className="w-full px-3 md:px-4 py-2 md:py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm" />
          ...
          <div>
            ...
            <textarea name="description" value={formData.description} onChange={handleInputChange} placeholder="Describe your campaign, what help is needed, and the impact it will have" maxLength="5000" rows="5" className="w-full px-3 md:px-4 py-2 md:py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none text-sm" />
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-900 mb-2">Cover Image</label>
            <div className="border-2 border-dashed border-slate-300 rounded-lg p-8 text-center hover:border-indigo-500 transition-colors cursor-pointer relative">
              <input type="file" accept="image/*" onChange={handleImageUpload} value="" className="absolute inset-0 opacity-0 cursor-pointer" />
              {formData.cover_image ? (
                <div className="relative inline-block">
                  <img src={formData.cover_image} alt="Cover preview" className="h-40 mx-auto mb-2 rounded object-cover" />
                  <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setFormData(prev => ({ ...prev, cover_image: null })); }} className="absolute -top-3 -right-3 bg-red-500 text-white rounded-full p-1.5 shadow-md hover:bg-red-600 transition-colors z-10">
                    <XIcon />
                  </button>
            ...
            <div className="border border-slate-300 rounded-lg p-4 bg-slate-50 relative">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 overflow-hidden">
                  <p className="text-sm font-medium text-slate-900 truncate">{verificationDocument ? verificationDocument.name : 'Select a document to upload...'}</p>
                  <p className="text-xs text-slate-500">{verificationDocument ? `${(verificationDocument.size / 1024 / 1024).toFixed(2)} MB` : 'PDF, JPG, PNG up to 5MB'}</p>
                </div>
                {verificationDocument && (
                  <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setVerificationDocument(null); }} className="relative z-10 p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-md transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[['category_name', 'Category', formData.category_id], ['subcategory_name', 'Subcategory', formData.subcategory_id]].map(([name, label, hasAi]) => (
              <div key={name}>
                <label className="block text-sm font-semibold text-slate-900 mb-2">{label} <span className="text-red-600">*</span></label>
                <div className="relative">
                  <input type="text" name={name} value={formData[name]} onChange={handleInputChange} placeholder="Awaiting analysis..." className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white" />
                  {hasAi && <AiBadge />}
                </div>
              </div>
            ))}
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-900 mb-2">Urgency Level <span className="text-red-600">*</span></label>
              <select name="urgency_level" value={formData.urgency_level} onChange={handleInputChange} className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-semibold text-slate-900 mb-2">Campaign Deadline (Optional)</label>
              <input type="datetime-local" name="deadline" value={formData.deadline} onChange={handleInputChange} className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500" />
            </div>
          </div>
          ...
          <div className="p-4 flex-1 overflow-y-auto space-y-4 text-green-400 font-medium">
            ...
            {analyzing && (
              <div className="flex items-center gap-2 text-yellow-400 mt-4 animate-pulse">
                <Loader2 className="w-3.5 h-3.5 animate-spin" /><span>Analyzing campaign text in real-time...</span>
              </div>
            )}
            {aiData && !analyzing && (
              <div className="animate-fade-in space-y-2 mt-4">
                ...
                <pre className="bg-slate-950 p-3 rounded-lg border border-slate-800 text-[11px] text-emerald-300 overflow-x-auto shadow-inner">{JSON.stringify(aiData, null, 2)}</pre>
                <div className="mt-2 space-y-1">
                  {[['extracted_goal', 'Auto-filled goal amount'], ['inferred_urgency', 'Auto-filled urgency level'], ['predicted_category', 'Auto-selected category']].map(([key, text]) => aiData[key] && (
                    <p key={key} className="text-indigo-300 flex items-center gap-1.5"><ShieldCheck className="w-3 h-3" />{text}</p>
                  ))}
                </div>
              </div>
            )}
            {refining && (
              <div className="flex items-center gap-2 text-purple-400 mt-4 animate-pulse">
                <Loader2 className="w-3.5 h-3.5 animate-spin" /><span>Refining description using LLM copywriter...</span>
              </div>
            )}
          </div>
        </div>
