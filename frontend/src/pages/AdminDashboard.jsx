  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        // Fetch vendors list
        const vendorsData = await apiService.getAdminVendors(profile.accessToken);
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <Button variant="secondary" size="md" onClick={() => navigate('/admin/stats')}>
            System Config
          </Button>
          <Button size="md" onClick={() => navigate('/admin/vendors')}>
            Verify Vendors
          </Button>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>New additions to the EmpathI network.</CardDescription>
        </CardHeader>
                  ...
                  {vendors.map((vendor, i) => (
                    <tr key={vendor.id || i} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-6 py-6 font-bold text-slate-900 group mr-1 uppercase tracking-tight">
                        {vendor.shop_name || vendor.name || 'Unknown'}
                      </td>
