...
const MatchResults = () => {
      ...
      try {
        ...
        if (!requestId) {
          ...
          if (requestHistory && requestHistory.length > 0) {
            // Get the most recent request (assuming it's the first one or last one depending on API order)
            const mostRecentRequest = Array.isArray(requestHistory)
              ? requestHistory[0]
              : requestHistory;
            requestId = mostRecentRequest.id || mostRecentRequest.request_id;
        ...
        const matchesData = await apiService.getMatches(profile.accessToken, requestId);
  ...
  const calculateMatchScore = (match) => {
    return match.relevance_score || 0;
  };

  const getDistance = (match) => {
    return match.distance_km ? `${match.distance_km} km` : 'N/A';
  };
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Stock levels are verified every 15 minutes via AVRE Sync.
                  </p>
                </div>
              </div>
            </div>
