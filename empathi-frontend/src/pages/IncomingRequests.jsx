...
const IncomingRequests = () => {
  ...
  const handleAccept = async (requestId, vendorId) => {
    try {
      await apiService.acceptMatch(profile.accessToken, requestId, vendorId);
      // Refresh list
      loadRequests();
    } catch (error) {
      ...
      <div className="grid gap-6">
                  ...
                  <div className="flex-grow p-8">
                     <div className="flex justify-between items-start mb-6">
                        <div>
                            <div className="flex items-center gap-3 mb-1">
                                <h3 className="text-xl font-display font-black text-slate-900 tracking-tight uppercase">{req.name}</h3>
                            ...
                            <p className="text-slate-500 font-medium text-sm flex items-center">
                                Requested by <span className="text-slate-900 font-bold ml-1.5 uppercase tracking-wide">{req.requester}</span>
                            </p>
                        </div>
                        <div className="text-right">
                           <p className="text-sm font-bold text-slate-900">{req.quantity}</p>
                           <p className="text-xs text-slate-400 font-medium">{new Date(req.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                        </div>
                     </div>
                     ...
                     <div className="grid grid-cols-2 md:grid-cols-3 gap-8 mb-8">
                        <div className="flex items-center space-x-3">
                           ...
                           <div>
                              ...
                              <p className="text-sm font-black text-slate-700">{req.distance}</p>
                           </div>
                        </div>
                        <div className="flex items-center space-x-3">
                           <div className="p-2 bg-slate-50 rounded-lg text-slate-400">
                           ...
                           <div>
                              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Required By</p>
                              <p className="text-sm font-black text-slate-700">Today, 5 PM</p>
                           </div>
                        </div>
                        <div className="hidden md:flex items-center space-x-3">
                           ...
                           <div>
                              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Market Priority</p>
                              <p className="text-sm font-black text-slate-700">Top 5% Demand</p>
                           </div>
                        </div>
                     </div>
                     ...
                     <div className="flex items-center gap-4 pt-4 border-t border-slate-50">
                        <Button 
                          ...
                          onClick={() => handleAccept(req.request_id, req.vendor_id)}
                          disabled={req.status !== 'pending'}
                        >
                           <Check className="w-5 h-5 mr-2" /> {req.status === 'pending' ? 'Accept Request' : 'Accepted'}
                        </Button>
        ...
        {requests.length === 0 && (
          <div className="py-24 text-center bg-white rounded-3xl border border-dashed border-slate-200">
             ...
             <p className="text-slate-500 max-w-xs mx-auto">New requests from AVRE will appear here in real-time.</p>
          </div>
        )}
      </div>
    </div>
