...
class CampaignRankerService:
    ...
    def _get_single_features(self, db: Session, c: 'Campaign', context: Dict[str, Any]=None) -> Dict[str, Any]:
        try:
            return {'goal_amount': c.goal_amount or 1000, 'raised_percentage': self._get_raised_percentage(c), 'days_active': (datetime.now() - c.created_at).days, 'urgency_level': self._encode_urgency(c.urgency_level), 'donor_count': len(c.donations), 'avg_donation': self._compute_avg_donation(c), 'recent_donations_7d': self._count_recent_donations(c, days=7), 'momentum': self._compute_momentum(c), 'is_verified': 1 if c.verified else 0, 'creator_trust_score': self._get_creator_trust_score(db, c.created_by), 'toxicity_score': getattr(c, 'toxicity_score', 0.0) or 0.0, 'category_confidence': getattr(c, 'category_confidence', 0.5) or 0.5, 'spam_risk_score': getattr(c, 'spam_risk_score', 0.0) or 0.0, 'category_match': self._compute_category_match(c, context), 'geographic_relevance': self._compute_geographic_relevance(c, context), 'created_recently': 1 if (datetime.now() - c.created_at).days < 7 else 0}
        except Exception as e:
            logger.warning(f"Error extracting features for campaign {getattr(c, 'id', 'unknown')}: {e}")
            return {'goal_amount': 1000, 'raised_percentage': 0.5, 'days_active': 0, 'urgency_level': 0.5, 'donor_count': 0, 'avg_donation': 0, 'recent_donations_7d': 0, 'momentum': 0, 'is_verified': 0, 'creator_trust_score': 0.5, 'toxicity_score': 0.0, 'category_confidence': 0.5, 'spam_risk_score': 0.0, 'category_match': 0.5, 'geographic_relevance': 0.5, 'created_recently': 0}
    ...
    def _compute_geographic_relevance(self, c: 'Campaign', ctx: Dict=None) -> float:
        ...
        if c.lat is not None and c.lng is not None and (ctx.get('user_lat') is not None) and (ctx.get('user_lng') is not None):
            ...
            if dist <= settings.ULTRA_PROXIMITY_THRESHOLD_KM:
                return 1.0
            if dist <= settings.PROXIMITY_THRESHOLD_KM:
                return 0.8
            if dist <= settings.MAX_MATCH_DISTANCE_KM:
                return 0.6
            return 0.3
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        try:
            return dict(zip(names, self.model.feature_importance()))
        except:
            return {}
