...
class HFServices:
    ...
    def _post_inference(self, model_id: str, payload: Any, timeout_s: int=15) -> Optional[Any]:
        ...
        try:
            ...
            logger.error('HF inference API Exception: %s', e)
        return None
    def _post_image(self, model_id: str, image_bytes: bytes) -> Optional[Any]:
        ...
        try:
            ...
            logger.error(f'HF API Exception ({model_id}): {e}')
        return None
    ...
    def analyze_campaign_comprehensive(self, text: str, historical_campaigns: List[Any]=None, taxonomy_str: str='') -> Dict[str, Any]:
        ...
        if res and 'choices' in res and (len(res['choices']) > 0):
            ...
            if '```' in raw_text:
                raw_text = raw_text.split('```json' if '```json' in raw_text else '```')[1].split('```')[0].strip()
            try:
    ...
    def refine_campaign_description(self, text: str) -> str:
        ...
        res = self._post_chat_completions({'model': 'Qwen/Qwen2.5-1.5B-Instruct:featherless-ai', 'messages': [{'role': 'system', 'content': 'You are a professional copywriter. Rewrite the given humanitarian or medical campaign description to make it more emotional, attention-grabbing, and compelling. Improve clarity and impact. Keep it realistic. Just return the new text, no intro, no markdown blocks, no outro.'}, {'role': 'user', 'content': text}], 'max_tokens': 1024, 'temperature': 0.7})
    ...
    def analyze_report(self, campaign_title: str, campaign_description: str, report_reason: str) -> str:
        if not self.api_key:
            return 'Mock AI Analysis: Checked report reason. No severe violations detected.'
        ...
        res = self._post_chat_completions({'model': 'Qwen/Qwen2.5-1.5B-Instruct:featherless-ai', 'messages': [{'role': 'system', 'content': sys_prompt}, {'role': 'user', 'content': user_prompt}], 'max_tokens': 128, 'temperature': 0.3})
    ...
    def audit_document_comprehensive(self, ocr_text: str, campaign_title: str, campaign_description: str, campaign_category: str, forensics: Dict[str, Any]) -> Dict[str, Any]:
        fallback = {'document_type': 'Invoice', 'is_authentic': True, 'trust_score': 75.0, 'fraud_probability': 0.15, 'billing_validation': {'stated_total': 0.0, 'computed_total': 0.0, 'sum_mismatch': False}, 'institution_validation': {'institution_name': 'Unknown', 'is_valid_institution': True, 'match_score': 1.0}, 'context_alignment': {'is_aligned': True, 'reasoning': 'Alignment assumed.'}, 'anomalies': []}
        if not self.api_key:
            return fallback
        sys_prompt = 'You are a professional document auditor and trust verification AI.\nAnalyze the extracted OCR text of a document, the campaign details, and forensic results, then output a JSON response verifying the document\'s authenticity, billing consistency, and category alignment.\n\nCRITICAL RULES:\n- Return ONLY valid JSON. Do not include markdown formatting or backticks (no ```json).\n- Be highly objective. If there are signs of tampering or severe mismatch, flag them.\n- Perform mathematical sum verification on the items in the OCR text. Check if the individual item prices sum up to the stated total.\n- For `trust_score`, return a float between 0.0 and 100.0.\n- For `fraud_probability`, return a float between 0.0 and 1.0.\n- For `is_authentic`, set to false if fraud_probability > 0.5 or there are severe mismatches.\n- For `context_alignment`, verify if the document supports the campaign\'s title/description/category.\n- OCR RESILIENCE RULE: Be highly resilient to minor OCR typos, character misreadings, and scanning artifacts (e.g. misreading \'2026\' as \'2070\' due to OCR font confusion, or \'$00\' as \'500\'). If a discrepancy is likely an OCR reading artifact rather than deliberate tampering, do not penalize the fraud_probability or trust_score.\n\nRequired JSON format:\n{\n  "document_type": "Medical Invoice|Tuition Receipt|ID Proof|Other",\n  "is_authentic": true|false,\n  "trust_score": float,\n  "fraud_probability": float,\n  "billing_validation": {\n    "stated_total": float,\n    "computed_total": float,\n    "sum_mismatch": true|false\n  },\n  "institution_validation": {\n    "institution_name": "string",\n    "is_valid_institution": true|false,\n    "match_score": float\n  },\n  "context_alignment": {\n    "is_aligned": true|false,\n    "reasoning": "string"\n  },\n  "anomalies": ["string"]\n}'
        user_content = f"CAMPAIGN DETAILS:\nTitle: {campaign_title}\nDescription: {campaign_description}\nCategory: {campaign_category}\n\nFORENSICS:\nEXIF Software Score: {forensics.get('exif_score', 1.0)}\nELA Mean: {forensics.get('ela_mean', 0.0)}\nELA Variance: {forensics.get('ela_variance', 0.0)}\nYOLO Logo Conf: {forensics.get('logo_conf', 0.0)}\nYOLO Signature Conf: {forensics.get('sig_conf', 0.0)}\nYOLO Stamp Conf: {forensics.get('stamp_conf', 0.0)}\n\nEXTRACTED DOCUMENT OCR TEXT:\n{ocr_text}"
        res = self._post_chat_completions({'model': 'Qwen/Qwen2.5-1.5B-Instruct:featherless-ai', 'messages': [{'role': 'system', 'content': sys_prompt}, {'role': 'user', 'content': user_content}], 'max_tokens': 512, 'temperature': 0.1})
        if res and 'choices' in res and (len(res['choices']) > 0):
            raw_text = res['choices'][0]['message']['content'].strip()
            if '```' in raw_text:
                raw_text = raw_text.split('```json' if '```json' in raw_text else '```')[1].split('```')[0].strip()
            try:
                ...
                if not match:
                    return fallback
                data = json.loads(match.group(0))
                return {'document_type': data.get('document_type', fallback['document_type']), 'is_authentic': bool(data.get('is_authentic', fallback['is_authentic'])), 'trust_score': float(data.get('trust_score', fallback['trust_score'])), 'fraud_probability': float(data.get('fraud_probability', fallback['fraud_probability'])), 'billing_validation': data.get('billing_validation', fallback['billing_validation']), 'institution_validation': data.get('institution_validation', fallback['institution_validation']), 'context_alignment': data.get('context_alignment', fallback['context_alignment']), 'anomalies': data.get('anomalies', fallback['anomalies'])}
            except Exception as e:
                logger.error(f'Failed to parse LLM Document Audit JSON: {e} | Raw text: {raw_text}')
        return fallback
