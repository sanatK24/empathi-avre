...
class TrustEngine:
    ...
    def compute_creator_trust(self, db: Session, creator_id: int) -> Optional[Dict[str, float]]:
        try:
            ...
            if not creator: logger.warning(f"Creator {creator_id} not found"); return None
            ...
            if not trust_profile: trust_profile = CampaignCreatorTrust(user_id=creator_id); db.add(trust_profile)
            ...
            db.commit(); db.refresh(trust_profile)
        ...
        except Exception as e: logger.error(f"Error computing trust for creator {creator_id}: {e}"); return None
    def _compute_fulfillment_probability(self, db: Session, creator_id: int) -> float:
        from models import Campaign, CampaignStatus, User
        try:
            creator = db.query(User).filter(User.id == creator_id).first()
            if not creator: return 0.5
            ...
            if not creator_campaigns: return 0.5 * (0.5 * age_factor + 0.5)
        ...
        except Exception as e: logger.warning(f"Error computing fulfillment probability: {e}"); return 0.5
    def _compute_fraud_risk(self, db: Session, creator_id: int) -> float:
        ...
        except Exception as e: logger.warning(f"Error computing fraud risk: {e}"); return 0.3
    def _extract_fraud_features(self, db: Session, creator_id: int) -> Optional[np.ndarray]:
        ...
        except Exception as e: logger.warning(f"Error extracting fraud features: {e}"); return None
    def _heuristic_fraud_score(self, db: Session, creator_id: int) -> float:
        ...
        try:
            ...
            score += 0.1 if (campaigns and np.var([c.goal_amount for c in campaigns]) > 1e6) else 0
    ...
    def _load_model(self):
        if os.path.exists(self.MODEL_PATH):
            ...
            except Exception as e: logger.warning(f"Could not load fraud model: {e}")
        return None
    def train_fraud_model(self, db: Session, positive_examples: list = None):
        ...
        try:
            ...
            if X.shape[0] < 10: logger.warning("Not enough training data for fraud model (need ≥10 examples)"); return False
            ...
            return True
        except Exception as e: logger.error(f"Fraud model training failed: {e}"); return False
    def _prepare_fraud_training_data(self, db: Session, positive_examples: list = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        except Exception as e: logger.error(f"Error preparing fraud training data: {e}"); return np.array([]), np.array([])
