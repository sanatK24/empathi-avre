...
class CampaignService:
    ...
    def create_campaign(db: Session, user: User, data: Any) -> Campaign:
        ...
        camp = Campaign(title=data.title, description=data.description, category_id=cat.id if cat else data.category_id, subcategory_id=data.subcategory_id, city=user.city, lat=user.lat, lng=user.lng, goal_amount=data.goal_amount, urgency_level=data.urgency_level, cover_image=data.cover_image, deadline=data.deadline, created_by=user.id, status=CampaignStatus.ACTIVE, ai_summary=ai_sum, category_tags=json.dumps([p_cat] if p_cat else []), category_confidence=0.7 if analysis.get('inferred_urgency') else 0.0, toxicity_score=tox, spam_risk_score=0.0, embedding_vector=json.dumps(embeds))
        db.add(camp)
        db.commit()
        db.refresh(camp)
        try:
    ...
    def get_recommendations(db: Session, user: User, limit: int=20) -> List[Dict[str, Any]]:
        ...
        for (c, s) in ranked:
            try:
                ...
                if tp and (not tp['is_fraud_flagged']):
                    trusted_ranked.append((c, s, tp['composite_trust_score']))
            except:
                trusted_ranked.append((c, s, 0.5))
        logger.debug('Applying fairness reranking')
        fair_ranked = fairness_engine_service.apply_diversity_constraint(fairness_engine_service.apply_fairness_reranking(db, [(c, s) for (c, s, _) in trusted_ranked], user.id))
        final_recs = []
        for (c, adj) in fair_ranked[:limit]:
            ...
            try:
                ...
                pass
            reasons = [r for (r, cond) in [('Matches your interests', c.category_id in user_cats), ('In your city', bool(user.city and c.city and (c.city.lower() == user.city.lower()))), ('Verified campaign', bool(c.verified)), ('High urgency', bool(c.urgency_level and c.urgency_level.value.lower() in ['high', 'critical']))] if cond]
            final_recs.append({'id': c.id, 'title': c.title, 'description': c.description, 'cover_image': getattr(c, 'cover_image', None), 'verified': bool(c.verified), 'ml_score': round(adj * 100, 1), 'trust_score': round(t_score * 100, 1), 'reason': ' • '.join(reasons) if reasons else 'Recommended for you', 'progress': round(c.raised_amount / c.goal_amount * 100 if c.goal_amount > 0 else 0, 1), 'category': c.taxonomy_category.name if c.taxonomy_category else 'General Aid', 'city': c.city, 'urgency_level': c.urgency_level.value if c.urgency_level else None, 'goal_amount': c.goal_amount, 'raised_amount': c.raised_amount, 'donor_count': len(c.donations)})
    ...
    def add_donation(db: Session, user: User, campaign_id: int, amount: float, anonymous: bool=False) -> Donation:
        ...
        if camp.raised_amount >= camp.goal_amount:
