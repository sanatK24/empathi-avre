...
def list_campaigns(skip: int=0, limit: int=20, category: Optional[str]=None, city: Optional[str]=None, db: Session=Depends(get_db)):
    query = db.query(Campaign).filter(((Campaign.status == CampaignStatus.ACTIVE) | (Campaign.status == CampaignStatus.COMPLETED)) & (Campaign.is_flagged == False))
    if category:
...
def get_campaign_stats(campaign_id: int, background_tasks: BackgroundTasks, db: Session=Depends(get_db)):
    ...
    return {'total_donations': total_count, 'average_donation': total_amount / total_count if total_count > 0 else 0, 'unique_donors': len(set((d.user_id for d in donations))), 'total_raised': total_amount}
...
def get_campaign_updates(campaign_id: int, db: Session=Depends(get_db), current_user: User=Depends(get_active_user)):
    ...
    return [{'id': u.id, 'campaign_id': u.campaign_id, 'created_by': u.created_by, 'content': u.content, 'image_url': u.image_url, 'is_pinned': u.is_pinned, 'created_at': u.created_at, 'creator': u.creator, 'comments_count': len(u.comments), 'likes_count': len(u.likes), 'is_liked_by_user': any((l.user_id == current_user.id for l in u.likes))} for u in updates]
...
def like_campaign_update(campaign_id: int, update_id: int, db: Session=Depends(get_db), current_user: User=Depends(get_active_user)):
    if not db.query(CampaignUpdate).filter(CampaignUpdate.id == update_id).first():
        raise HTTPException(status_code=404, detail='Update not found')
...
async def verify_campaign(campaign_id: int, file: Optional[UploadFile]=File(None), db: Session=Depends(get_db), current_user: User=Depends(get_active_user)):
    campaign = db.query(Campaign).filter(Campaign.id == campaign_id).first()
    if not campaign:
        raise HTTPException(status_code=404, detail='Campaign not found')
    if campaign.created_by != current_user.id and current_user.role.upper() != 'ADMIN':
    ...
    if file and file.filename:
        ...
        try:
            ...
            return JSONResponse(status_code=400, content={'error': 'Invalid file'})
        try:
            public_url = storage_service.upload_document(file_bytes, filename, file.content_type)
            campaign.verification_doc_url = public_url
            db.commit()
        except Exception as storage_err:
            return JSONResponse(status_code=500, content={'error': f'Failed to upload document to storage: {str(storage_err)}'})
    else:
        if not campaign.verification_doc_url:
            return JSONResponse(status_code=400, content={'error': 'No document'})
        try:
            import requests
            response = requests.get(campaign.verification_doc_url, timeout=15)
            if response.status_code != 200:
                return JSONResponse(status_code=400, content={'error': 'Failed to fetch existing document'})
            file_bytes = response.content
            filename = campaign.verification_doc_url.split('/')[-1]
            if len(filename) > 37 and filename[36] == '-':
                filename = filename[37:]
        except Exception as download_err:
            return JSONResponse(status_code=500, content={'error': f'Failed to fetch existing document: {str(download_err)}'})
    try:
        ...
        try:
            ...
            audit_repo.log(db, action='verify_campaign', user_id=current_user.id, resource_type='campaign', resource_id=campaign_id, details=f"Ran AI verification on campaign. Trust Score: {result['trust_score']}, Status: {result['status']}.", severity=severity)
        except Exception as e:
