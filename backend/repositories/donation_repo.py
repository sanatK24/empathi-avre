...
class DonationRepo(BaseRepo[Donation]):
    ...
    def get_by_campaign(self, db: Session, campaign_id: int) -> List[Donation]:
        return db.query(Donation).filter(Donation.campaign_id == campaign_id, Donation.status == DonationStatus.COMPLETED).order_by(Donation.created_at.desc()).all()

    def get_total_raised(self, db: Session, campaign_id: int) -> float:
        return db.query(func.sum(Donation.amount)).filter(Donation.campaign_id == campaign_id, Donation.status == DonationStatus.COMPLETED).scalar() or 0.0
