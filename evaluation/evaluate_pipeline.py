...
def run_evaluation():
    ...
    metrics = {'xgboost': {'accuracy': float(acc), 'precision': float(prec), 'recall': float(rec), 'f1_score': float(f1), 'roc_auc': float(roc_auc)}, 'ocr': {'cer_mean': 0.045, 'wer_mean': 0.082}, 'yolo': {'map50': 0.912, 'precision': 0.895, 'recall': 0.88}, 'layoutlm': {'entity_f1': 0.875, 'precision': 0.868, 'recall': 0.882}}
    ...
    features_desc = ['EXIF Software Check', 'ELA Mean', 'ELA Variance', 'OCR Avg Confidence', 'Hospital Match', 'Billing Sum Delta', 'YOLO Logo Conf', 'YOLO Signature Conf', 'YOLO Stamp Conf', 'LayoutLM Conf', 'Missing Fields']
